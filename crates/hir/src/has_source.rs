...
impl HasSource for Field {
    ...
    fn source<DB: HirDatabase>(self, sema: &Semantics<'_, DB>) -> Option<InFile<Self::Ast>> {
        ...
        let src = var.child_source(sema.db.upcast(), &mut None);
...
impl HasSource for Struct {
    type Ast = ast::Struct;
    fn source<DB: HirDatabase>(self, sema: &Semantics<'_, DB>) -> Option<InFile<Self::Ast>> {
        Some(self.id.source(sema.db.upcast()))
    }
}
...
impl HasSource for Macro {
    ...
    fn source<DB: HirDatabase>(self, sema: &Semantics<'_, DB>) -> Option<InFile<Self::Ast>> {
        match self.id {
            MacroId::Macro2Id(it) => {
                Some(it.source(sema.db.upcast()).map(ast::Macro::MacroDef).map(Either::Left))
            }
            MacroId::MacroRulesId(it) => {
                Some(it.source(sema.db.upcast()).map(ast::Macro::MacroRules).map(Either::Left))
            }
            MacroId::ProcMacroId(it) => Some(it.source(sema.db.upcast()).map(Either::Right)),
        }
    }
}
...
impl HasSource for TypeOrConstParam {
    ...
    fn source<DB: HirDatabase>(self, sema: &Semantics<'_, DB>) -> Option<InFile<Self::Ast>> {
        let child_source = self.id.parent.child_source(sema.db.upcast(), &mut None);
